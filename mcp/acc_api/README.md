...
The server runs as a background systemd user service — it starts on boot and auto-restarts on crashes.

```bash
# Status and live logs
systemctl --user status agriai-search
journalctl --user -u agriai-search -f

# Restart after code changes
systemctl --user restart agriai-search

# Stop / start manually
systemctl --user stop agriai-search
systemctl --user start agriai-search
```
...
| `DB_NAME` | `agriai` | Database name |
| `COLLECTION_NAME` | `questions` | Collection name |
| `VECTOR_INDEX_NAME` | `review_questions_vector_index` | Atlas Vector Search index name |
