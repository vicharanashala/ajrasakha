...
async def _get_weather_forecast_logic(
...
async def _get_current_weather_logic(
    lat: Optional[float] = None,
    lon: Optional[float] = None,
    zip_code: Optional[str] = None,
...
async def _get_location_info_logic(
    lat: float,
    lon: float,
...
@mcp.tool()
async def get_weather_or_location(
    lat: Optional[float] = None,
    lon: Optional[float] = None,
    zip_code: Optional[str] = None,
    country_code: Optional[str] = "IN",
    ...
    # Feature flags
    get_current_weather: bool = False,
    get_forecast: bool = False,
    get_location_info: bool = False,
    ...
    """
    Unified Weather and Location Tool.
    
    Use this tool to get current weather, weather forecast, or detailed location information.
    You must set at least one of the feature flags to True.
    ...
    Args:
        ...
        get_current_weather: Set to True to fetch current weather
        get_forecast: Set to True to fetch weather forecast
        get_location_info: Set to True to fetch reverse geocoding info (requires lat/lon)
        
        forecast_days: Number of days for forecast (1-16, default 3)
        location_type: Filter for location info ('country', 'city', 'street', etc.)
        
        use_fallback: Force use of secondary API sources where applicable
    """
    results = {}
    
    if not (get_current_weather or get_forecast or get_location_info):
        return {
            "success": False,
            "error": "No action requested",
            "hint": "Set at least one of get_current_weather, get_forecast, or get_location_info to True"
        }
        
    # Validation for location info which strictly requires lat/lon
    if get_location_info:
        if lat is None or lon is None:
             # If we have zip code, we might want to resolve it first? 
             # For now, following strict requirement of the underlying tool which takes lat/lon.
             # But wait, the underlying current_weather logic can resolve zip->coords. 
             # However, _get_location_info_logic takes lat/lon directly.
             # If the user only provided zip, and wants location info, we might be stuck unless we resolve it.
             # Let's try to resolve it if possible using the other helpers if we are also getting weather?
             # Or just return error for now as the underlying tool requires it.
             # To be safe and robust:
             if zip_code:
                 # Attempt resolution via openweathermap logic if we can?
                 # Actually _get_weather_from_openweathermap does resolution internally but returns weather.
                 # Let's just error for now to keep it simple as per original tool contract.
                 pass
             
    # Execute requested actions
    
    if get_current_weather:
        results["current_weather"] = await _get_current_weather_logic(
            lat=lat, lon=lon, zip_code=zip_code, country_code=country_code,
            units=units, lang=lang, use_fallback=use_fallback
        )
        
    if get_forecast:
        results["forecast"] = await _get_weather_forecast_logic(
            lat=lat, lon=lon, zip_code=zip_code, country_code=country_code,
            units=units, lang=lang, days=forecast_days, use_fallback=use_fallback
        )

    if get_location_info:
        if lat is not None and lon is not None:
            results["location_info"] = await _get_location_info_logic(
                lat=lat, lon=lon, location_type=location_type, lang=lang
            )
        else:
            results["location_info"] = {
                "success": False, 
                "error": "get_location_info requires lat and lon coordinates"
            }
