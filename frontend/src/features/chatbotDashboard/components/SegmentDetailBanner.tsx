...
export function SegmentDetailBanner({ seg, onClose }: { seg: Segment; onClose: () => void }) {
  return (
    <div style={{ background: "rgba(58,170,90,0.08)", border: "1.5px solid #3AAA5A", borderRadius: 12, padding: "14px 16px", marginBottom: 16, display: "flex", gap: 24, alignItems: "flex-start", animation: "slideIn 0.25s ease" }}>
      <div style={{ flex: 1 }}>
      ...
      <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#3AAA5A", fontSize: 18, lineHeight: 1, padding: "0 4px" }}>×</button>
    </div>
  );
}
