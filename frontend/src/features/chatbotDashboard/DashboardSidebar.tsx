...
export const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
  ...
  const healthScoreColor =
    healthScore >= 75 ? "text-[#1E7A3C]" : healthScore >= 50 ? "text-[#854F0B] dark:text-[#EF9F27]" : "text-[#A32D2D]";

  return (
    <aside className="w-[210px] bg-(--card) border-r border-(--border) shrink-0 flex flex-col overflow-y-auto overflow-x-hidden relative">
      {/* ── NAV SECTIONS ── */}
      ...
      {/* ── HEALTH SCORE FOOTER ── */}
      <div className="absolute bottom-0 left-0 w-full px-4 py-3 border-t border-(--border) bg-(--card)">
        ...
        <div className="flex items-center gap-2 mt-1">
          <div className="flex-1 h-1 bg-(--muted) rounded-sm overflow-hidden">
            <div
              className="h-full bg-[#3AAA5A] rounded-sm transition-all duration-500"
  ...
  return (
    <div
      ...
      className={`
        ...
        ${active
          ? "border-l-[#3AAA5A] bg-[#EAF6EC] dark:bg-[#1a3a24] text-[#1E7A3C] dark:text-[#4adc64] font-medium"
          : "border-l-transparent text-(--muted-foreground) font-normal hover:bg-(--accent)"
        }
      `}
    >
  ...
  return (
    <div
      ...
      className={`
        ...
        transition-all duration-150 border-l-2
        ${active
          ? "border-l-[#3AAA5A] bg-[#f0faf2] dark:bg-[#1a3a24] text-[#1E7A3C] dark:text-[#4adc64] font-medium"
          : "border-l-transparent text-(--muted-foreground) font-normal hover:bg-(--accent)"
        }
      `}
    >
...
const SidebarSectionLabel: React.FC<{ children: React.ReactNode }> = ({
  ...
  <div className="text-[10px] font-medium text-(--muted-foreground) px-4 mt-4 mb-1 uppercase tracking-wider">
    {children}
  </div>
);
