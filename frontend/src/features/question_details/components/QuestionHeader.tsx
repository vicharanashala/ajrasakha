...
export const QuestionHeader = ({
  ...
  const handleHold = () => {
    ...
    setConfirmDialog({
      open: true,
      type: question.isOnHold ? "unhold" : "hold",
    });
  };
  const doHold = async () => {
    try {
      ...
      toast.success(
        `Question ${question.isOnHold ? "released from hold" : "put on hold"} successfully`,
      );
      goBack();
    } catch (error) {
      console.error(error);
  ...
  const { mutateAsync: updateQuestion, isPending: isCancellingDuplicate } =
    useUpdateQuestion();
  ...
  const handleCancelDuplicate = async () => {
    ...
    try {
      ...
      toast.dismiss(toastId);
      toast.success(
        "Duplicate cancelled. Question reopened. Please refresh the page to see updated changes.",
      );
  ...
  const currentAnswerText =
    finalAnswer?.answer ?? lastHistoryWithAnswer?.answer?.answer ?? "";
  const currentAnswerSources =
    finalAnswer?.sources ?? lastHistoryWithAnswer?.answer?.sources ?? [];
  ...
  const formattedTime =
    diffMs !== null && diffMs > 0 ? formatMs(diffMs) : "N/A";
  const moderatorFormattedTime =
    moderatorDiffMs !== null && moderatorDiffMs > 0
      ? formatMs(moderatorDiffMs)
      : "N/A";
          ...
          <div className="flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-start sm:justify-end sm:flex-shrink-0">
            <div className="flex flex-wrap justify-end gap-2">
              ...
              {question.question?.trim() &&
                !isEnglishCharacters(question.question) && (
                ...
                <Button
                  ...
                  onClick={() =>
                    checkDuplicate(question._id!, {
                      onSuccess: (res) => {
                        toast.success(
                          res?.message ?? "Duplicate check complete.",
                        );
                      },
      ...
      >
        <DialogContent className="!max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            This will reopen the question (status → open). Please provide a
            reason for cancelling the duplicate.
          </p>
          <Textarea
            value={cancelReason}
          ...
          <div className="space-y-1.5">
            <p className="text-sm font-medium">
              Do you want to turn on auto allocation?
            </p>
            <div className="flex gap-2">
              <Button
                type="button"
        ...
        <DialogContent className="!max-w-[65vw] max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between gap-3 pr-8">
              ...
              {question.status === "closed" &&
                (referenceAnswerText || currentAnswerText) && (
                  <Button
                    size="sm"
                                    ...
                                    {currentAnswerSources.map((s, i) => (
                                      ...
                                      >
                                        <span className="font-medium text-foreground">
                                          {i + 1}.{" "}
                                          {s.sourceName
                                            ? `${s.sourceName}${s.page != null ? ` (p. ${s.page})` : ""}`
                                            : "Source"}
                                        </span>
                                        {s.source &&
                          ...
                          {previousText ? (
                            <>
                              ...
                              <div className="pt-2 border-t mt-auto">
                                ...
                                {question.referenceQuestionData?.sources &&
                                question.referenceQuestionData.sources.length >
                                  0 ? (
                                  <ul className="space-y-2">
                                    {question.referenceQuestionData.sources.map(
                                      (s, i) => (
                                        <li
                                          key={i}
                                          className="text-xs flex flex-col gap-0.5"
              ...
              {!compareMode && (
                <div>
                  ...
                  <div className="border rounded-md p-3 bg-muted/20">
                    {question.referenceQuestionData.sources &&
                    question.referenceQuestionData.sources.length > 0 ? (
                      <ul className="space-y-2">
