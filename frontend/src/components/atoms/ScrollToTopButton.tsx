...
export interface ScrollToTopButtonProps {
  ...
  /** Scroll distance in pixels before the button appears. Defaults to 200. */
  threshold?: number;
...
export function ScrollToTopButton({
  ...
  const handleScroll = useCallback(() => {
    ...
    const elScrollTop = el ? el.scrollTop : 0;
    const winScrollTop =
      window.scrollY ||
    ...
    if (el && elScrollTop > winScrollTop && el.scrollHeight > el.clientHeight) {
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            "fixed bottom-6 right-6 z-50 flex items-center justify-center pointer-events-auto",
            className,
          )}
        >
          <Tooltip>
