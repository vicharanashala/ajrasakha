...
export const RequestListItem = ({
  ...
  return (
    <>
      {/* Diff Modal — same as RequestCard */}
      <div className="fixed inset-0 flex items-start justify-center z-50 p-6 pointer-events-none">
        ...
        {/* History Modal — same as RequestCard */}
        <Dialog open={responseOpen} onOpenChange={() => setResponseOpen(false)}>
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-foreground">
                                  {response.reviewerName || "Unknown Reviewer"}
                                </span>
                                <Badge
                                  variant="outline"
                                ...
                                >
                                  ...
                                  <span className="ml-1">{response.role}</span>
                                </Badge>
                              </div>
      ...
      <div
        id={id}
        className={cn(
          "group relative flex items-center gap-4 rounded-lg border bg-card px-4 py-3 transition-all duration-200",
          isHighlighted
      ...
      >
        ...
        {/* Created */}
        <div className="w-36 flex-shrink-0 text-xs text-muted-foreground hidden md:block">
          {new Date(req.createdAt).toLocaleString()}
        </div>

        {/* Status */}
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium border whitespace-nowrap flex-shrink-0 ${statusClass(req?.status)}`}
        >
          {req?.status?.toUpperCase() || "N/A"}
        </span>
