    ...
    <div className="space-y-4 m-1">
      {allKeys.map((key) => {
        ...
        if (bIsArrObj || aIsArrObj) {
          ...
          return (
            <div key={key}>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">{key}</p>
              <div className="flex gap-2 mb-1.5">
        ...
        return (
          <div key={key} className="grid grid-cols-[1fr_1fr_1fr] gap-1 text-xs border-t border-border/30 pt-2">
            ...
            <span className={`font-mono px-1.5 py-0.5 rounded ${changed ? "bg-red-50 text-red-800 dark:bg-red-950/60 dark:text-red-300" : "text-foreground"} text-wrap`}>{bStr || "—"}</span>
            <span className={`font-mono px-1.5 py-0.5 rounded ${changed ? "bg-green-50 text-green-800 dark:bg-green-950/60 dark:text-green-300" : "text-foreground"} text-wrap`}>{aStr || "—"}</span>
          </div>
        );
      })}
    </div>
    ...
    <div className="space-y-1.5">
      {Object.entries(flat).map(([k, v]) => (
        <div key={k} className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-[11px] font-mono bg-muted px-2 py-0.5 rounded border border-border/60">{renderScalar(v)}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="bg-red-50/40 dark:bg-red-950/20 border border-red-300 dark:border-red-800 rounded p-3 space-y-2">
      ...
      {outcome.errorCode && (
        <div>
          <p className="text-[11px] text-muted-foreground">Code:</p>
          <pre className="text-xs bg-red-50 dark:bg-red-900 p-2 rounded overflow-x-auto">{outcome.errorCode}</pre>
        </div>
      )}
      {outcome.errorName && (
        <div>
          <p className="text-[11px] text-muted-foreground">Name:</p>
          <pre className="text-xs bg-red-50 dark:bg-red-900 p-2 rounded overflow-x-auto">{outcome.errorName}</pre>
        </div>
      )}
      {outcome.errorMessage && (
        <div>
          <p className="text-[11px] text-muted-foreground">Message:</p>
          <pre className="text-xs bg-red-50 dark:bg-red-900 p-2 rounded overflow-x-auto text-wrap">{outcome.errorMessage}</pre>
        </div>
      )}
      {outcome.errorStack && (
        <div>
          <p className="text-[11px] text-muted-foreground">Stack Trace:</p>
          <pre className="text-xs bg-red-50 dark:bg-red-900 p-2 rounded overflow-x-auto whitespace-pre-wrap">{outcome.errorStack}</pre>
        </div>
      )}
    </div>
  );
    ...
    <Card>
      <CardContent className="pt-4 pb-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Tooltip>
              <TooltipTrigger><Badge variant="category">{entry.category}</Badge></TooltipTrigger>
              <TooltipContent><p>Category</p></TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger><Badge variant="action">{entry.action}</Badge></TooltipTrigger>
              <TooltipContent><p>Action</p></TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger><Badge variant="role">{entry.actor.role}</Badge></TooltipTrigger>
            ...
            <Tooltip>
              ...
              <TooltipContent><p>Outcome</p></TooltipContent>
            </Tooltip>
          </div>
        </div>
      </CardContent>
...
// ─── Expandable Detail Panel (shared between table row and card) ──────────────
 
function DetailPanel({ entry }: { entry: AuditEntry }) {
  return (
    <div className="bg-muted/30 px-4 py-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Left */}
      <div className="space-y-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Actor</p>
          <table className="text-xs w-full">
            <tbody>
              {[
                ["Name", entry.actor.name || "-"],
                ...
                <tr key={label}>
                  ...
                  <td className="py-1 break-all">{val}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Right */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">Changes</p>
        <DiffViewer before={entry.changes?.before ?? {}} after={entry.changes?.after ?? {}} />
      </div>
    </div>
  );
}
...
function DetailRow({ entry }: { entry: AuditEntry }) {
  const [open, setOpen] = useState(false);
  const isSuccess = entry.outcome.status === "SUCCESS";
  ...
  return (
    <>
      ...
      <TableRow className="hover:bg-muted/40 transition-colors hidden lg:table-row">
        ...
        <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{fmtDate(entry.createdAt)}</TableCell>
        <TableCell><Badge variant={isSuccess ? "success" : "danger"}>{entry.outcome.status}</Badge></TableCell>
        <TableCell>
          <Button variant="outline" size="sm" onClick={() => setOpen((o) => !o)} className="h-7 px-2 text-xs gap-1">
      ...
      <TableRow className="hidden lg:table-row">
        <TableCell colSpan={7} className="p-0 border-b">
          <div className={`transition-all duration-300 overflow-hidden ${open ? "max-h-[1200px] opacity-100" : "max-h-0 opacity-0"}`}>
      ...
      {/* Mobile card */}
      <div className="lg:hidden rounded-xl border border-border/60 overflow-hidden bg-card">
        <div
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                <Badge variant="action">{entry.action}</Badge>
                <Badge variant="category">{entry.category}</Badge>
                <Badge variant={isSuccess ? "success" : "danger"}>{entry.outcome.status}</Badge>
              </div>
            </div>
          </div>
...
const AuditPage = () => {
  ...
  enum AuditAction {
    ...
    ORGANIZATION_BULK_CREATE = "Import Organizations",
    SEND_OUTREACH_REPORT = "Send Outreach Report",
    DOWNLOAD = "Download",
    APPROVE_ANSWER = "Approve Answer",
    REROUTE_ANSWER = "Reroute Answer",
  ...
  enum reverseActionEnum {
    ...
    "Add Crop" = "ADD_CROP",
    "Update Crop" = "UPDATE_CROP",
    "Add Organization" = "ADD_ORGANIZATION",
    "Update Organization" = "UPDATE_ORGANIZATION",
    "Delete Organization" = "DELETE_ORGANIZATION",
    "Import Organizations" = "ORGANIZATION_BULK_CREATE",
    "Send Outreach Report" = "SEND_OUTREACH_REPORT",
    "Download" = "DOWNLOAD",
    "Approve Answer" = "APPROVE_ANSWER",
    "Reroute Answer" = "REROUTE_ANSWER",
  ...
  const actionObject = {
    QUESTION: [AuditAction.ALL, AuditAction.QUESTION_ADD, AuditAction.QUESTION_UPDATE, AuditAction.QUESTION_DELETE, AuditAction.QUESTION_BULK_CREATE, AuditAction.QUESTION_BULK_UPDATE, AuditAction.QUESTION_BULK_DELETE, AuditAction.REALLOCATE_QUESTIONS],
    EXPERTS_CATEGORY: [AuditAction.ALL, AuditAction.EXPERTS_AUTO_ALLOCATE, AuditAction.SELECT_EXPERT, AuditAction.DELETE_EXPERT, AuditAction.EXPERTS_ADD_COMMENT],
    EXPERTS_MANAGEMENT: [AuditAction.ALL, AuditAction.BLOCK_EXPERT, AuditAction.UNBLOCK_EXPERT],
    REQUEST_QUEUE: [AuditAction.ALL, AuditAction.CHANGE_STATUS, AuditAction.DELETE_REQUEST],
    ANALYTICS: [AuditAction.ALL, AuditAction.ANALYTICS_EXPORT_PDF],
  ...
  // Shared filter controls — rendered in both desktop toolbar and mobile drawer
  const FilterControls = () => (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="gap-1 text-xs w-full sm:w-auto justify-start">
            <ArrowBigDownDashIcon size={14} />
            {category ? AuditCategory[category as keyof typeof AuditCategory] : "Category"}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-56 p-3 space-y-2">
          <DropdownMenuRadioGroup value={category ?? undefined} onValueChange={(value) => { setCategory(value); setAction(null); }}>
            <DropdownMenuRadioItem value="">All</DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="QUESTION">Question</DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="EXPERTS_CATEGORY">Experts Category</DropdownMenuRadioItem>
      ...
      {category && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="gap-1 text-xs w-full sm:w-auto justify-start">
              <ArrowBigDownDashIcon size={14} />
              {action ? AuditAction[action as keyof typeof AuditAction] : "Action"}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56 p-3 space-y-2">
            <DropdownMenuRadioGroup value={action ?? undefined} onValueChange={(value) => setAction(value)}>
              {actionObject[category as keyof typeof actionObject]?.map((act) => (
                <DropdownMenuRadioItem key={act} value={reverseActionEnum[act as keyof typeof reverseActionEnum]}>
                  {act}
                </DropdownMenuRadioItem>
              ))}
            </DropdownMenuRadioGroup>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56 p-3 space-y-2">
          <DropdownMenuRadioGroup value={outComeStatus} onValueChange={(value) => setOutComeStatus(value)}>
            <DropdownMenuRadioItem value="SUCCESS">Success</DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="FAILED">Failed</DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="PARTIAL">Partial</DropdownMenuRadioItem>
          </DropdownMenuRadioGroup>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56 p-3 space-y-2">
          <DropdownMenuRadioGroup value={order} onValueChange={(value) => setOrder(value as "asc" | "desc")}>
        ...
        <DropdownMenuContent align="end" className="w-56 p-3 space-y-2">
          <div className="space-y-1">
            ...
            <Input
              type="datetime-local"
              value={startDateTime ?? ""}
              onChange={(e) => { setStartDateTime(e.target.value || undefined); setPage(1); }}
              className="w-full border rounded-md px-2 py-1 text-xs bg-background"
            />
          </div>
          <div className="space-y-1">
            ...
            <Input
              type="datetime-local"
              value={endDateTime ?? ""}
              onChange={(e) => { setEndDateTime(e.target.value || undefined); setPage(1); }}
              className="w-full border rounded-md px-2 py-1 text-xs bg-background"
            />
          </div>
          {(startDateTime || endDateTime) && (
            <Button variant="ghost" size="sm" className="w-full text-xs mt-1" onClick={() => { setStartDateTime(undefined); setEndDateTime(undefined); setPage(1); }}>
              Clear filters
            </Button>
          )}
        </DropdownMenuContent>
  ...
  return (
    <div className="px-3 sm:px-4 md:px-6 py-4 space-y-4 w-full max-w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-base sm:text-lg font-semibold truncate">Audit Trail</h1>
          {!isLoading && (
            <span className="shrink-0 text-xs bg-purple-50 text-purple-800 border border-purple-200 dark:bg-purple-950 dark:text-purple-200 dark:border-purple-800 px-2 py-0.5 rounded-full font-medium">
              {total} entries
            </span>
          )}
        </div>
 
        {/* Right: refresh + view toggle + mobile filter button */}
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {isRefreshing ? <Loader2 className="animate-spin w-4 h-4" /> : <><RefreshCw className="w-4 h-4" /><span className="hidden sm:inline">Refresh</span></>}
          </Button>
          ...
          <Button
            ...
            className="gap-1 text-xs lg:hidden relative"
          >
            <SlidersHorizontal size={14} />
            <span className="sm:inline">Filters</span>
            {activeFilterCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 h-4 w-4 rounded-full bg-purple-600 text-white text-[9px] flex items-center justify-center font-bold">
                {activeFilterCount}
              </span>
            )}
          </Button>
          ...
          <div className="flex border rounded-lg overflow-hidden">
            <button
              onClick={() => setView("detail")}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs transition-colors ${view === "detail" ? "bg-muted font-medium text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
            >
              <LayoutGrid size={13} />
            ...
            <button
              onClick={() => setView("compact")}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs border-l transition-colors ${view === "compact" ? "bg-muted font-medium text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
            >
              <Table2 size={13} />
      ...
      {filtersOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          {/* Drawer */}
          <div className="relative bg-background rounded-t-2xl border-t border-border p-4 space-y-3 max-h-[80vh] overflow-y-auto">
            ...
            {activeFilterCount > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full text-xs text-red-600 dark:text-red-400 mt-1"
                onClick={() => { setCategory(null); setAction(null); setOutComeStatus(""); setStartDateTime(undefined); setEndDateTime(undefined); setPage(1); }}
              >
                Clear all filters
              </Button>
            )}
            <Button size="sm" className="w-full mt-1" onClick={() => setFiltersOpen(false)}>
              Apply
            </Button>
          </div>
        </div>
      ...
      {!isLoading && !error && view === "compact" && (
        <div className="space-y-2">
          ...
          {entries.map((entry) => <CompactCard key={entry._id} entry={entry} />)}
        </div>
      )}
      ...
      {!isLoading && !error && view === "detail" && (
        <>
          ...
          <div className="lg:hidden space-y-2 w-full">
            {entries.length === 0 ? (
              <div className="text-center text-sm text-muted-foreground py-12">No audit entries found.</div>
            ) : (
              entries.map((entry) => <DetailRow key={entry._id} entry={entry} />)
            )}
          </div>
