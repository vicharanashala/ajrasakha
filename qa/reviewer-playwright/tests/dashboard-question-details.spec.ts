...
test.describe("Reviewer dashboard to question-detail vertical slice", () => {
  ...
  test("PERM-001 expert login resolves the reviewer role and reaches /home", async ({
    ...
    expect(
      syncResponse.ok(),
      `POST /auth/sync failed with ${syncResponse.status()}`,
    ).toBeTruthy();
  ...
  test("DASH-001 authenticated shell exposes the role-supported question navigation", async ({
    ...
    await expect(
      authenticatedPage.getByRole("tab", { name: "All Questions" }),
    ).toBeVisible();
  ...
  // Implementation: QuestionsPage, QuestionDetails.
  // Verifies that the selected question details remain visible after a browser reload.
  test("QDET-002 selected question details remain visible after a browser reload", async ({
    authenticatedPage,
    dashboardPage,
    questionDetailsPage,
  }) => {
    ...
    // Verify the correct question is initially opened.
    await questionDetailsPage.expectQuestionText(selected.question);

    // Reload the page.
    await authenticatedPage.reload();

    // The selected question should remain open after reload.
    await questionDetailsPage.expectQuestionText(selected.question);
  });
  ...
  test("QDET-004 question metadata uses the full-detail response and implemented fallbacks", async ({
    ...
    expect(await questionDetailsPage.metadataValue("State")).toBe(
      data.details?.state || "-",
    );
    expect(await questionDetailsPage.metadataValue("District")).toBe(
      data.details?.district || "-",
    );
    expect(await questionDetailsPage.metadataValue("Crop")).toBe(
      data.details?.crop || "-",
    );
  ...
  test("QDET-006 allocation queue renders every returned reviewer and a derived status", async ({
    ...
    if (queue.length > 0) {
      expect(
        await questionDetailsPage.availableAllocationStatuses(),
      ).not.toHaveLength(0);
    }
  });
  ...
  test("QDET-011 in-page Exit returns from question detail to the question list", async ({
    ...
    await expect(
      dashboardPage.questionTable.or(dashboardPage.emptyState.first()),
    ).toBeVisible();
  });
});
