...
export class ExpertQuestionDetailsPage {
  ...
  constructor(private readonly page: Page) {
    ...
    this.allocationHeading = page.getByRole("heading", {
  ...
  async expectStatus(status: string): Promise<void> {
    await expect(
      this.header.getByText(status.replace("_", " "), { exact: true }),
    ).toBeVisible();
  }
  ...
  async metadataValue(label: string): Promise<string> {
    ...
    return (
      await labelNode.locator("..").locator("span").nth(1).innerText()
    ).trim();
  }

  async expectAllocationQueue(
    queue: Array<{ name?: string; email?: string }>,
  ): Promise<void> {
    await expect(this.allocationHeading).toBeVisible();
    if (queue.length === 0) {
      await expect(
        this.page.getByRole("heading", { name: "No Experts Allocated" }),
      ).toBeVisible();
      return;
    }
    ...
    for (const reviewer of queue) {
      if (reviewer.name) {
        await expect(
          this.page.locator(`[title=${JSON.stringify(reviewer.name)}]`).first(),
        ).toBeVisible();
      } else if (reviewer.email) {
        await expect(
          this.page
            .locator(`[title=${JSON.stringify(reviewer.email)}]`)
            .first(),
        ).toBeVisible();
      }
    }
