...
export class ChatbotService extends BaseService implements IChatbotService {
  ...
  async getDashboard(
    ...
    try {
      const [
        ...
        this.chatbotRepository.getDailyQueryCounts(
          days,
          source,
          undefined,
          userType,
        ...
        this.chatbotRepository.getMonthlyQueryCounts(
          source,
          undefined,
          userType,
        ),
  ...
  async getDailyQueryCounts(
    ...
    try {
      return await this.chatbotRepository.getDailyQueryCounts(
        days,
        source,
        undefined,
        userType,
      ...
      throw new InternalServerError(
        `Failed to fetch daily query counts: ${error}`,
      );
    }
  }
  ...
  async getWeeklyQueryCounts(source = 'vicharanashala', userType = 'all') {
    try {
      ...
      throw new InternalServerError(
        `Failed to fetch weekly query counts: ${error}`,
      );
    }
  }
