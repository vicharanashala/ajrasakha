...
export const aiConfig = {
  ...
  plantNetProject: env('PLANTNET_PROJECT') || 'all',
  plantNetMock: env('PLANTNET_MOCK') === 'true',
  plantNetRelayBaseUrl: env('PLANTNET_RELAY_BASE_URL'),
  plantScanLocalImageBaseUrl:
    env('PLANT_SCAN_LOCAL_IMAGE_BASE_URL') ||
    `http://127.0.0.1:${Number(env('APP_PORT')) || 8080}`,
  ...
  agerntServerPort:
    Number(env('AGENT_SERVER_PORT')?.trim()) || 9017,

  openAIServerIP: env('OPENAI_SERVER_IP'),
  openAIServerPort:
    Number(env('OPENAI_SERVER_PORT')?.trim()) || 8080,

  whatsAppServerPort:
    Number(env('WHATSAPP_SERVER_PORT')?.trim()) || 2026,

  aiInitialAnswerGenerateUrl:
    env('AI_INITIAL_ANSWER_GENERATE_URL'),
  ...
  vicharanashala_api_token:
    env('VICHARANASHALA_API_TOKEN'),

  gdbServerIP: env('GDB_SERVER_IP') || 'localhost',
  gdbServerPort:
    Number(env('GDB_SERVER_PORT')?.trim()) || 8110,
  ...
  accAgentTimeout:
    Number(env('ACC_AGENT_TIMEOUT')?.trim()) || 10000,
};