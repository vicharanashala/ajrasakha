...
// Questions we temporarily close in beforeAll to free STF capacity. Restored in afterAll.
...
beforeAll(async () => {
  ...
  process.env.INTERNAL_API_KEY = 'e2e-alloc-ordering-key';
  ...
  // Close leftover open WHATSAPP/AJRASAKHA questions that have STF experts in
  // their queue — they count against MAX_TIME_BOUND=1 and would block G1.
  ...
  if (stfExperts.length > 0) {
    ...
    if (leftoverSubs.length > 0) {
      ...
      const leftoverActive = await questionsCol
        .find({
          _id: { $in: leftoverQIds },
      ...
      if (leftoverActive.length > 0) {
        const toClose = leftoverActive.map((q: any) => q._id);
        await questionsCol.updateMany({ _id: { $in: toClose } }, { $set: { status: 'closed' } });
        temporarilyClosedIds.push(...toClose);
        console.log(`[setup] Temporarily closed ${toClose.length} leftover allocated time-bound question(s).`);
      }
    }
  }
  // Also close unallocated (queue=[]) WHATSAPP/AJRASAKHA questions from previous
  // incomplete runs — the cron would consume STF capacity on those first.
  const unallocSubs = await submissionsCol.find({ queue: { $size: 0 } }).toArray();
  if (unallocSubs.length > 0) {
    const unallocQIds = unallocSubs.map((s: any) => s.questionId);
    const unallocTB = await questionsCol
      .find({
        _id: { $in: unallocQIds },
        source: { $in: ['WHATSAPP', 'AJRASAKHA'] },
      ...
      .toArray();
    if (unallocTB.length > 0) {
      const toClose2 = unallocTB.map((q: any) => q._id);
      await questionsCol.updateMany({ _id: { $in: toClose2 } }, { $set: { status: 'closed' } });
      temporarilyClosedIds.push(...toClose2);
      console.log(`[setup] Also closed ${toClose2.length} unallocated time-bound question(s) from previous runs.`);
    }
  }
...
afterAll(async () => {
  ...
  if (db && temporarilyClosedIds.length) {
    ...
    console.log(`[teardown] Restored ${temporarilyClosedIds.length} temporarily-closed question(s).`);
  }
