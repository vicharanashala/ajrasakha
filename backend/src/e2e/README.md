...
| Suite | File | Tests | Last run (2026-06-16) | What it covers |
|-------|------|------:|----------------------|----------------|
...
| Question CRUD | `question/QuestionCreate.e2e.test.ts` | 8 | ❌ 2/8 | Moderator create / get / update / delete / bulk-delete (OUTREACH source) |
...
| WhatsApp ingestion | `whatsapp/WhatsAppQuestion.e2e.test.ts` | 18 | ❌ 4/18 | Full ingestion pipeline: auth, GDB duplicate paths, LLM filter, thread validation + retry |
| AjraSakha ingestion | `ajrasakha/AjrasakhaQuestion.e2e.test.ts` | 9 | ❌ 4/9 | AJRASAKHA-specific fields (userId from `@CurrentUser`, notification type), representative pipeline cases |
| Manual allocation | `manual-allocation/ManualAllocation.e2e.test.ts` | 10 | ❌ 8/10 | `POST /allocate-experts` + `DELETE /allocation` on an OUTREACH question |
| Auto allocation | `auto-allocation/AutoAllocation.e2e.test.ts` | 44 | ❌ 36/44 | AGRI_EXPERT background queue, preference scoring, toggle, time-bound allocation (WHATSAPP/AJRASAKHA), capacity, reviewer, concurrent guard |
