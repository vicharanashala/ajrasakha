...
// const PROD_DB_NAME = 'agriai';

// // Helper function to convert IDs cleanly safely mapping them if possible
...
//     });
//     console.log('🛡️  Production database transaction started.');

//     // ID Maps tracking (Old Staging ID String -> New Prod ObjectId)
//     const maps = {
//       users: new Map(),
//       questions: new Map(),
//       answers: new Map(),
...
//     const stagingQuestions = await stagingDb.collection('questions').find({}).toArray();
...
//         // Map deep authors history array
...
//         // Map deep referenceQuestionDetails nested array
...
//     }
//     console.log(`✅ Questions complete: ${stagingQuestions.length}`);
...
//     if (stagingAnswers.length > 0) {
//       const answerOps = stagingAnswers.map(ans => {
//         const newId = new ObjectId();
...
//         // Map nested modifications history array
...
//     }
//     console.log(`✅ Answers complete: ${stagingAnswers.length}`);
...
//     if (stagingReviews.length > 0) {
//       const reviewOps = stagingReviews.map(rev => {
//         const newId = new ObjectId();
...
//     }
//     console.log(`✅ Reviews complete: ${stagingReviews.length}`);

//     // ==========================================
//     // 5. QUESTION SUBMISSIONS PHASE
//     // ==========================================
//     console.log('📝 Migrating Question Submissions...');
//     const stagingSubmissions = await stagingDb.collection('question_submissions').find({}).toArray();

//     if (stagingSubmissions.length > 0) {
//       const submissionOps = stagingSubmissions.map(sub => {
//         const newId = new ObjectId();
...
//     }
//     console.log(`✅ Submissions complete: ${stagingSubmissions.length}`);

//     // ==========================================
//     // 6. POST-UPDATE: FIX MAPPED MODERATOR USER ASSIGNMENTS
//     // ==========================================
//     // If staging users had 'assignedQuestionIds' arrays containing active question references,
//     // those denormalized IDs inside production users should point to the new question IDs.
//     console.log('\n🔄 Updating denormalized assignedQuestionIds references inside Production users...');
//     const prodUsersWithAssignments = await prodDb.collection('users').find({ 
//       assignedQuestionIds: { $exists: true, $ne: null } 
//     }, { session: prodSession }).toArray();

//     for (const user of prodUsersWithAssignments) {
//       let changed = false;
//       const updatedAssignments = user.assignedQuestionIds.map(assign => {
//         const oldQStr = assign.questionId?.toString();
//         if (oldQStr && maps.questions.has(oldQStr)) {
//           changed = true;
//           return { ...assign, questionId: maps.questions.get(oldQStr) };
//         }
//         return assign;
//       });
...
//           { _id: user._id },
//           { $set: { assignedQuestionIds: updatedAssignments } },
//           { session: prodSession }
//         );
//       }
//     }
//     console.log('✅ Denormalized user fields updated.');
...
//     console.log('🎉 MIGRATION SUCCESSFULLY COMPLETED & COMMITTED WITH ZERO LEAKS!');
