"""Tests for multilingual query coverage."""
...
def test_no_missing_or_placeholder_queries():
    """All 30 scenarios in the data artifact must contain language slots for all target languages."""
    ...
    # Check each language slot exists
    ...
    for entry in scenarios:
        ...
        for lang in expected_langs:
            assert lang in queries, f"Scenario {sid} missing query slot for language {lang}"
            q = queries.get(lang, "")
            if q:
...
def test_query_translations_match_scripts():
    """Basic sanity check that filled native translations actually contain native script."""
    payload = json.loads(_QUERIES_PATH.read_text(encoding="utf-8"))
    scenarios = payload.get("scenarios", [])
    
    # Quick heuristic check for script ranges for languages with provided queries
    script_check = {
