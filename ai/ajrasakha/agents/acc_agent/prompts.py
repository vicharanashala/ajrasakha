...
14. "language_preference": Farmer's preferred language for communication, only if explicitly stated. Do not infer it from the language used in the transcript. Use null if not mentioned.
15. "years_of_experience": Farmer's farming experience in completed years, as a non-negative integer, only if explicitly stated. Do not derive it from age. Use null if not mentioned.
16. "highest_education": Farmer's own highest educational qualification, only if explicitly stated. Use null if not mentioned.
17. "smartphones_at_home": Number of smartphones in the farmer's home, as a non-negative integer, only if explicitly stated. Use null if not mentioned.

""" + DOMAIN_TAXONOMY + """

CRITICAL INSTRUCTIONS:
- You MUST output ONLY a valid JSON object with the keys "query", "state", "district", "crop", "standardized_domains", "name", "phone", "age", "gender", "village", "block", "primary_crop", "secondary_crops", "language_preference", "years_of_experience", "highest_education", and "smartphones_at_home".
